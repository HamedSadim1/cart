...
const CartItem = ({ cart }: CartItemProps) => {
  ...
  const { setDecrease, setIncrease, setRemoveCart } = useCartQueryStore();

  return (
    <article className="cart-item">
      <ModalImage small={img} large={img} alt={title} />
      <div>
        <h4>{title}</h4>
        <h4 className="item-price">${price}</h4>
        {/* remove button */}
        <button className="remove-btn" onClick={() => setRemoveCart(id)}>
          remove
        </button>
      </div>
      <div>
